...
int validate_map(t_game *game)
{
	...
	for (i = 0; i < game->map_height; i++)
	{
		...
		if ((int)strlen(game->map[i]) != game->map_width)
			return (printf("Error: Map not rectangular\n") && 0);
	...
	// Validate counts
	if (player != 1)
		return (printf("Error: Must have exactly one player\n") && 0);
	if (exit != 1)
		return (printf("Error: Must have exactly one exit\n") && 0);
	if (collect < 1)
		return (printf("Error: Must have at least one collectible\n") && 0);
