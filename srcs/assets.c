...
void *scale_image(void *mlx, void *original, int width, int height, int n_size)
{
	...
	while (++params[3] < n_size)
	{
		...
		while (++params[4] < n_size)
			if (params[3] * width / n_size * width + params[4] * width / n_size >= 0 && params[3] * width / n_size * width + params[4] * width / n_size < (width * height))
				new_data[params[3] * n_size + params[4]] = orig_data[params[3] * width / n_size * width + params[4] * width / n_size];
	}
	return (new_img);
}
...
void *flip_image_horizontal(void *mlx, void *img)
{
	...
	while (++params[0] < height)
	{
		...
		{
			*(unsigned int *)(data[0] + params[0] * params[1] + (width - 1 - params[2]) * 4) = *(unsigned int *)(data[1] + params[0] * params[1] + params[2] * 4);
		}
	}
	return (new_img);
}
